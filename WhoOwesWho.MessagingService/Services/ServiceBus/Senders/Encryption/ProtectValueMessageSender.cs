...
namespace WhoOwesWho.UserService.Services.ServiceBus.Senders.Encryption
{
    ...
    public class ProtectValueMessageSender : EventServiceSenderBase, IProtectValueMessageSender
    {
        ...
        {
            _processor = client.CreateProcessor(EncryptionQueues.MessagingProtectResponse);
        ...
        public async Task<string> SendAsync(ProtectValueRequestModel request)
        {
            var task = await SendRequestAsync<ProtectValueRequestModel, string>(
                request,
                EncryptionQueues.MessagingProtectRequest,
                EncryptionQueues.MessagingProtectResponse);
