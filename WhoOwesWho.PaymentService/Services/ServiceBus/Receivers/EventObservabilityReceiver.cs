...
{
    public sealed class EventObservabilityReceiver : BackgroundService
    {
        private readonly ServiceBusProcessor _successProcessor;
        private readonly ServiceBusProcessor _failedProcessor;

        public EventObservabilityReceiver(ServiceBusClient client)
        {
            _successProcessor = client.CreateProcessor(
                topicName: ServiceBusTopics.MessagingTopics.PaymentEventDispatchSucceeded,
                subscriptionName: "authentication-observability");

            _failedProcessor = client.CreateProcessor(
                topicName: ServiceBusTopics.MessagingTopics.PaymentEventDispatchFailed,
                subscriptionName: "authentication-observability");
        }
        ...
        {
            var evt = args.Message.Body.ToObjectFromJson<PaymentDispatchEvent>();

            Console.WriteLine(
                $"[AUTH] Credentials dispatched OK | Type={evt?.Type} | User={evt?.Name}");
        ...
        {
            var evt = args.Message.Body.ToObjectFromJson<PaymentDispatchFailedEvent>();

            Console.WriteLine(
                $"[AUTH] Credentials dispatch FAILED | Type={evt?.Type} | User={evt?.Name} | Reason={evt?.Reason}");
