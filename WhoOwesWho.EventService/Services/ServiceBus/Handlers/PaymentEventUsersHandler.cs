...
{
    public class PaymentEventUsersHandler(IMessageResolverService resolver) : IServiceBusMessageHandler
    {
        public string QueueName => EventQueues.PaymentEventUsersRequest;
        ...
        public async Task<object?> HandleAsync(ServiceBusReceivedMessage request)
        {
            ...
            return await resolver.GetEventUsersAsync(input!.ApiKey, input.UserOrEventId!, input.Active);
        }
    }
}
